...
export const NavItems = (role: string) => {
	...
	return role === "Jumento" ? professorItems : studentItems;
};

export default function Sidebar({
	session_token_role,
}: { session_token_role: string }) {
				...
				>
					<TooltipProvider delayDuration={0}>
						<Tooltip>
								...
								>
									...
									{isSidebarExpanded && (
										<div className="flex flex-col hidden md:block md:visible">
											<span className="text-white font-medium">Clovis</span>
											<br />
											<span className="text-gray-400 text-sm">
												{session_token_role === "professor"
													? "Professor"
													: "Estudante"}
											</span>
										</div>
							...
							<TooltipContent
								side="right"
								className="px-3 py-1.5 text-xs bg-gray-800 text-white"
							>
								<span>
									Clovis -{" "}
									{session_token_role === "professor"
					...
					{isSidebarExpanded && (
						<ChevronDown
							size={16}
							className="text-white hidden md:block md:visible"
						/>
					)}
				</div>
				...
				<TooltipTrigger asChild>
					...
					>
						...
						>
							{icon}
							<span
								className={isSidebarExpanded ? "md:block hidden" : "hidden"}
							>
								{label}
							</span>
						</div>
